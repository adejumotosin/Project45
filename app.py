...
def load_acled_data(countries, start_date, end_date, event_types, min_fatalities):
    """Fetch ACLED data with caching"""
    all_data = []
    ...
    for idx, country in enumerate(countries):
        ...
        try:
            ...
            response = requests.get("https://api.acleddata.com/acled/read", params=params, timeout=30)
            data = response.json()
...
def is_eid_al_fitr(date):
    ...
    try:
        hijri_date = Gregorian(date.year, date.month, date.day).to_hijri()
        # Eid is first few days of Shawwal (month 10)
...
def is_eid_al_adha(date):
    ...
    try:
        hijri_date = Gregorian(date.year, date.month, date.day).to_hijri()
        # Eid al-Adha is 10th of Dhul Hijjah (month 12)
...
with st.spinner("Loading ACLED data..."):
    df = load_acled_data(selected_countries, start_date, end_date, selected_event_types, min_fatalities)
...
st.markdown("---")
st.caption(f"""
**Data Sources**: ACLED, Open-Meteo | **Built with**: Streamlit, Plotly, SciPy  
